...
void KNNLayer<Dtype>::Reshape(
  ...
  if (bottom.size() == 1) {
	...
	neighbor_indices_.Reshape(label_shape);
	non_neighbor_indices_.Reshape(label_shape);
    if (top.size() >= 3) {
      // id for neighbors
      top[2]->ReshapeLike(neighbor_indices_);
    }
    if (top.size() >= 4) {
      // id for non-neighbors
      top[3]->ReshapeLike(non_neighbor_indices_);
    }
  } else {
...
void KNNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (bottom.size() == 1) {
    ...
    Dtype* n_idx_data = neighbor_indices_.mutable_cpu_data();
    Dtype* nn_idx_data = non_neighbor_indices_.mutable_cpu_data();
    ...
    for (int i = 0; i < batch_size; ++i) {
      ...
      n_idx_data[i] = n_idx;
      nn_idx_data[i] = nn_idx;
    }
    if (top.size() >= 3) top[2]->ShareData(neighbor_indices_);
    if (top.size() >= 4) top[3]->ShareData(non_neighbor_indices_);
  } else {

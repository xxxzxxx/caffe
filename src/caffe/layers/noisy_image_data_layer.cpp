...
NoisyImageDataLayer<Dtype>::~NoisyImageDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void NoisyImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  const int new_height = this->layer_param_.noisy_image_data_param().new_height();
  const int new_width  = this->layer_param_.noisy_image_data_param().new_width();
  const bool is_color  = this->layer_param_.noisy_image_data_param().is_color();
  string root_folder = this->layer_param_.noisy_image_data_param().root_folder();

  CHECK((new_height == 0 && new_width == 0) ||
      (new_height > 0 && new_width > 0)) << "Current implementation requires "
      "new_height and new_width to be set at the same time.";
  // Read the file with filenames and labels
  const string& source = this->layer_param_.noisy_image_data_param().source();
  ...
  if (this->layer_param_.noisy_image_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
        this->layer_param_.noisy_image_data_param().rand_skip();
  ...
  const int batch_size = this->layer_param_.noisy_image_data_param().batch_size();
  if (crop_size > 0) {
...
template <typename Dtype>
void NoisyImageDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
void NoisyImageDataLayer<Dtype>::InternalThreadEntry() {
  ...
  const int crop_size = this->layer_param_.transform_param().crop_size();
  const bool is_color = noisy_image_data_param.is_color();
  string root_folder = noisy_image_data_param.root_folder();
